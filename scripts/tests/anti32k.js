import { world, ItemStack, MinecraftEnchantmentTypes, MinecraftItemTypes } from 'mojang-minecraft';
const bannedItems = [
    'minecraft:portal',
...
world.events.tick.subscribe(() => {
    for(const player of world.getPlayers()) {
        ...
        for(let i = 0; i < inventory.size; i++) {
            ...
            if(bannedItems.includes(item.id)) inventory.setItem(i, new ItemStack(MinecraftItemTypes.air));
            const itemEnchantments = item.getComponent('enchantments').enchantments;
            for(let ench of allEnchantments) if(itemEnchantments.hasEnchantment(ench) > ench.maxLevel) inventory.setItem(i, new ItemStack(MinecraftItemTypes.air));
        }
    }
});